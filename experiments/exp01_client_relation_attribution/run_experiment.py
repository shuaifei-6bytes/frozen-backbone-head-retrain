...
def run_seed(
    ...
    relation_description = assignment_description(split, full_assignments, neutral_assignments, records)
    ...
    write_json(seed_dir / "client_split.json", relation_description["source_pool_allocation"])
    write_json(seed_dir / "relation_distribution.json", relation_description)
    if relation_description["paired_control_status"]["design_compliance"] != "strict_same_subject_background_swap":
        logger.warning(
            "Approximate distribution intervention only: no same-subject background-swap asset is available."
        )
    ...
    initial_checkpoint = seed_dir / "init_model.pt"
    ...
    metrics["design_compliance"] = relation_description["paired_control_status"]
    ...
    write_json(
        seed_dir / "artifact_hashes.json",
        {
            ...
            for path in (
                ...
                seed_dir / "client_split.json",
                seed_dir / "relation_distribution.json",
